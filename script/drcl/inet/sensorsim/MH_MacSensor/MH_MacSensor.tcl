...
cd [mkdir -q drcl.comp.Component /multiHop]
...
for {set i 0} {$i < [expr $sink_id + 1]} {incr i} {
	...
	! mac setNode_num_  $nid       ;#Nicholas: set the MAc address
	! wphy setNid       $nid
	! mobility setNid   $nid
	! id setDefaultID   $nid
	...
	connect mobility/.report@ -and /multiHop/tracker/.node@
	connect wphy/down@ -to /multiHop/channel/.node@

	! /multiHop/channel attachPort $i [! wphy getPort .channel]
...
for {set i [expr $sink_id + 1]} {$i < [expr $node_num - $target_node_num]} {incr i} {
	...
	connect app/.getNeighbor@ -and /multiHop/nodetracker/.multiHop@
	#*****************
	...
	! /multiHop/chan attachPort $i [! phy getPort .channel]

	# connect the nodes to the propagation model
	connect phy/.propagation@ -and /multiHop/seismic_Prop/.query@
...
if { $target_node_num == 0 } {
	...
	for {set i [expr $node_num - $target_node_num]} {$i < $node_num} {incr i} {
		...
		connect phy/down@ -to /multiHop/chan/.node@
...
for {set i [expr $sink_id + 1]} {$i < $node_num} {incr i} {
	...
	connect n$i/mobility/.report_sensor@ -and /multiHop/nodetracker/.node@
...
#***********************************************
proc neighborUpdate { } {
   global sink_id node_num count sensorList delNodes target_node_num

   #puts "AT THE START OF NEIGHBORuPDATE New Size of List is: [llength $sensorList]"
   ...
   for {set i 0 } {$i < [llength $sensorList]} {incr i} {
     #extract the current element
      ...
      if { $remEnergy <= 0 } {
        ...
        #puts "Sensor$newSID is dead Updating upstream sensors"
	
        #puts "\nSelf ID: $newSID"
        #puts "\nSensor Neighbor: $newNID"
	#puts "$newX"
	#puts "$newY"
	#puts "$newZ"
	#puts "$newDist"
 ...
 #     puts "Index is: $index"
  ...
  # puts "New Size of List is: [llength $sensorList]"
 #  puts "\n"
} 
...
script "neighborUpdate" -at 15.0 -period 0.25 -on $sim

#************For Matlab plotting************************
#script "file_output" -at 5999.0 -period 25.0 -on $sim

#the following line is only used in validation!
#script "getCurrEnergy" -at 14.99 -on $sim
