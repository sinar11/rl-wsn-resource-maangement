...
! nodetracker setGrid 1000.0 0.0 1000.0 0.0
...
! tracker setGrid 1000.0 0.0 1000.0 0.0 100.0 100.0
...
#                                maxX maxY maxZ minX minY minZ dX dY dZ
    ! mobility setTopologyParameters 1000.0 1000.0 0.0 0.0 0.0 0.0 100.0 100.0 0.0
...
if { $target_node_num == 0 } {
	...
	for {set i [expr $node_num - $target_node_num]} {$i < $node_num} {incr i} {
		...
		! mobility setTopologyParameters 600.0 600.0 100.0 100.0 0.0 0.0 100.0 100.0 0.0
...
! n0/mobility setPosition 0.0 700.0 600.0 0.0
...
for {set i [expr $sink_id + 1]} {$i < [expr $node_num - $target_node_num]} {incr i} {
   ! n$i/mobility setPosition 0.0 [expr 50+rand()*650] [expr 50+rand()*600] 0.0     
}
...
script {! n0/app subscribe 10 100.0 800.0 100.0 800.0 200000.0 53.0 5.0 20000.0 10} -at 3.0 -on $sim
