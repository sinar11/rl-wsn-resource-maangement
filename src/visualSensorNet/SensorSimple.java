 ...
 *
 * This class creates all the components that make up a sensor node.
 *
...
public class SensorSimple {
    ...
    */
	void setTopo(double xmin_, double xmax_, double ymin_, double ymax_, double dx_,double dy_){
			xmin = xmin_;
			xmax = xmax_;
			ymin = ymin_;
			ymax = ymax_;
    ...
    */
     ...
     * @param env
     */
	public SensorSimple(int nid, int sink_id, Environnement env, int AppType_, double xCoord_, double yCoord_, double zCoord_)
    {
       // Attention ! j pour les indices de tableaux i pour la nid
        ...
        app.setCoherentThreshold(1000.0);   //threshold at which the sensor could no longer comprehend signals

        if ((AppType_ == LEACH) || (AppType_ == MH_CSMA)) {
            app.setNn_(env.getNn_());
        }

        /*If running multi-hop mode then we must connect the application
          layer and nodetracker components together*/
        ...
        phy.getPort(".toAgent"/*phy.TO_AGENT_PORT_ID*/).connectTo( agent.getPort(".fromPhy"/*agent.FROM_PHY_PORT_ID*/ ));

        // sensoragent-> sensorapp
        agent.getPort(".toSensorApp"/*agent.TO_SENSOR_APP_PORT_ID*/).connectTo(app.getPort(".fromSensorAgent"/*app.FROM_SENSOR_AGENT_PORT_ID */));

        //connexion externe : sensor channel -> sensorphy
        env.chan.attachPort(nid,phy.getPort(".channel"/*phy.CHANNEL_PORT_ID*/));

        // connexion externe : propagation model -> sensor
        phy.getPort(".propagation"/* phy.PROPAGATION_PORT_ID*/).connect( env.seismic_prop.getPort(".query"));
        ...
        wireless_agent.getPort(".toSensorApp").connectTo( app.getPort(".fromWirelessAgent"/*app.FROM_WIRELESS_AGENT_PORT_ID*/));
        ...
        Mac mac;
        if ((AppType_ == OH_CSMA)|| (AppType_ == OH_TDMA)
            || (AppType_ == MH_CSMA) || (AppType_ == LEACH)) {
            mac = new Mac_CSMA();
            mac.setID("MacSensor");
            ...
            mac.setID("mac");
            /*NICHOLAS:
             Because of the fact that RTS and CTS are short frames, it reduces
             the overhead of collisions, since they are recognized faster than
             it would be recognized if the whole packet was to be transmitted
             (this is true if the packet is significantly bigger than the RTS),
             so the MAC802.11 standard allows for short packets to be transmitted
             without the RTS/CTS transaction, and this is controlled per station
             by the RTSThreshold. (therefore below any packet that is 40 bytes
             or less will not require the use of RTS/CTS.
            */
        ...
        if ((AppType_ == OH_CSMA)|| (AppType_ == OH_TDMA)
            ...
            }else if (AppType_ == OH_TDMA) {
                wphy.setOneHopModeTDMA(true);
            }else if (AppType_ == MH_CSMA) {
                wphy.setMultiHopMode(true);
            }else if (AppType_ == LEACH) {
        ...
        /**
         * Nicholas:
         * Add a route entry directly back to the sink node.
        ...
        env.channel.attachPort(nid,wphy.getPort(".channel" /*wphy.CHANNEL_PORT_ID*/));
        ...
        mobility.getPort(".report_sensor"/*mobility.REPORT_SENSOR_PORT_ID*/).connect( env.nodetracker.getPort(".node"/*env.nodetracker.NODE_PORT_ID*/));
        phy.getPort(".mobility"/*phy.MOBILITY_PORT_ID*/ ).connect( mobility.getPort( ".query"));
    }
}