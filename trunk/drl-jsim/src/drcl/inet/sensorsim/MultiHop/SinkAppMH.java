 ...
 *
 * This class acts as the application layer for the sink when using the multi-hop scheme.
 * It simply listens on the channel for incomming data and plots graph to update
 * results for the end operators.
 *
*/

public class SinkAppMH extends SensorApp implements drcl.comp.ActiveComponent
{
    ...
    */
	public synchronized void recvSensorPacket(Object data_)
    {
		if ( data_ instanceof SensorPacket ) {
            ...
            double latency = getTime() - ((MH_Packet)spkt.getBody()).getSendTime();
            ...
            ***********************/
            ...
            //**********************
            long target_nid = ((SensorAppAgentContract.Message)((MH_Packet)spkt.getBody()).getBody()).getTargetNid();
            lastSeenSNR = ((SensorAppAgentContract.Message)((MH_Packet)spkt.getBody()).getBody()).getSNR();
    ...
    /**
     * Function which determines the total number of packets
     * the base station has received.
     * @return
     */
